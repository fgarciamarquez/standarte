...
- **24/09/2026**: `transformOroBody` reconoce «Ferias y sectores», «Documentación técnica» y Pat también por su TÍTULO (`FAIRS_HINT`/`DOC_HINT`), no solo por la posición: antes, en cuerpos con otra estructura (Lyon, Perpiñán, Badajoz, Zaragoza, Ciudad Real, Silleda, Ourense, Trujillo y muchas versiones traducidas) se rotulaba «Cómo trabajamos», «Tipos» o «Diseño y construcción» como documentación técnica y salían H2 duplicados. Los apartados del recinto van con la familia de montaje. `CITY_KW_LEAD` suma «empresa de stands en X» (y equivalente por idioma). Reparto resultante en el cuerpo ES: diseño 3-4, montaje 3-4, «stands para ferias» 2-3 (5 en Silleda, Ourense, Trujillo y Badajoz, que no tienen apartado de documentación); «constructor de stands» se deja a las paralelas.
- **Enlace ficha de feria → ciudad** (24/09/2026): en la respuesta directa de cada ficha (`cityLink` en `Feria.svelte`) el texto rota por feria (hash estable del slug) entre «diseño de stands en X», «montaje de stands en X» y «diseño y montaje de stands en X» (y equivalentes en los 11 idiomas). Antes todas decían «diseño y construcción de stands para ferias en X». Reparto ES: 155/141/148 de 444 fichas. Motivo: las fichas posicionan en 1.ª-2.ª posición y son la principal fuente de autoridad de las páginas de ciudad.
- **Revisión SEO de Lisboa (25/09/2026)**: 417 URL en el sitemap, 0 problemas (canonical, robots, hreflang, lastmod = dateModified). SCM Conference (Quinta do Frade, Sobral de Monte Agraço) cuelga de Lisboa como municipio satélite y se enlaza desde «Ferias y sectores» en 11 idiomas. Las páginas de proyecto (`/proyectos/<id>` y ja) emiten ya `lastmod`/`dateModified` con `projectFreshness` (`seoFreshness.js`, último cambio real 18/09/2026); antes no llevaban fecha.
- **Caída de Lisboa (26/09/2026)**: comprobación técnica en vivo sin fallos (200, index, canonical, Googlebot, sitemap). Causa probable: el cambio del 22/09 quitó «construção/construcción» de título, H1 y primer apartado (búsqueda muy habitual en Portugal) y el piloto del 24/09 hizo competir a la provisional. Medidas: `CITY_H1_FULL_SECTIONS` (`h2Seo.js`, solo `lisboa` de momento) → H1 y <title> «Diseño, construcción y montaje de stands en X» en 11 idiomas (`cityH1For`); la provisional de Lisboa vuelve a su versión anterior al piloto (sin FAQ). No tocar Lisboa en 2 semanas; si se recupera, valorar extender `CITY_H1_FULL` al resto de plazas.
- **Todos los términos (26/09/2026, decisión del usuario: sumar términos, nunca quitar)**: `CITY_H1` de las 67 principales pasa a «Diseño, construcción y montaje de stands para ferias en X» (11 idiomas; se retira la excepción `CITY_H1_FULL_SECTIONS`); `CITY_KW_LEAD` suma «construcción de stands en X»; la rotación de enlaces ficha → ciudad tiene 4 variantes (diseño / construcción / montaje / diseño, construcción y montaje: 105/120/113/107 de 445) y la etiqueta del pilar en el clúster de la ficha lleva la forma completa. El 22/09 se había sustituido «Diseño y construcción de stands para ferias» por «Diseño y montaje»: fue un error, porque quitó «construcción» y «para ferias».
...
- **Piloto de contenido completo (24/09/2026)** en las paralelas de Bilbao y Madrid (Lisboa salió del piloto el 26/09 y volvió al formato ligero) (es/en y pt en Lisboa; `scratchpad/pilot/`): estructura inspirada en una página competidora de Bilbao (pacpublicidad) pero con datos verdaderos: intro + 4 beneficios, «qué construimos» con 4 H3, CTA intermedio «¿Tienes una feria próxima…?», 4 razones, método en 4 pasos, proyectos reales enlazados (/proyectos/<id>) y FAQ propia (campo `faqs` en `builderSeoData`, FAQPage; H2 de FAQ con la expresión de la paralela vía `builderFaqH2`). Sin testimonios: no hay ninguno guardado y no se inventan. Medir 2-3 semanas antes de extenderlo.
...
- **Carrusel de renders en paralelas y páginas de ciudad** (23-24/09/2026): `RenderFader.svelte` (fundido de 1 s, 3 s por imagen, ciclo infinito, sin vídeos) entre el primer y el segundo párrafo del cuerpo; `Site.svelte` parte `bodyHtml` tras el primer `</p>` (`faderSplit`). Los renders viven en `src/lib/galleryRenders.js`, compartidos con la Galería; para añadir uno basta con meterlo ahí y en `static/img/galeria-renders/`.
- **Paralelas de las 37 ferias de Madrid** (23/09/2026, `scratchpad/mad/apply.mjs` + `d1-3.mjs`): mismo esquema que Zaragoza pero en formato ligero —intro, 3 H2 «Constructor de stands para <F>: …» (qué exige la feria, qué construimos con `<ul>` de 3 tipos, resto de IFEMA con 4 ferias en rotación + ficha + página de Madrid) y CTA—; texto propio por feria, es + en (`stand_builder_<f>`), taller San Fernando de Henares, indexables.
