...
async function safeTranslate(text, to) {
  ...
  while (attempts < 5) {
    try {
      ...
      if (e.message.includes('Too Many Requests') || e.statusCode === 429) {
        attempts++;
        console.log(`Rate limited, waiting ${attempts * 2} seconds...`);
...
async function processProjects() {
  ...
  for (let i = 0; i < projects.length; i++) {
    ...
    // Skip if already has genuine ko translation
    if (proj.title && proj.title.ko && !proj.title.ko.includes('한국어 번역')) {
      continue;
    }
    ...
    try {
      ...
      // Save every 10 projects to not lose progress
      if (i % 10 === 0) {
  ...
  const newContent = topPart + JSON.stringify(projects, null, 2) + bottomPart;
  fs.writeFileSync('src/lib/projectData.js', newContent, 'utf8');
  console.log('projectData.js translated successfully.');
}
...
async function processSEO() {
  ...
  console.log('richSeoData.js translated successfully.');
}
