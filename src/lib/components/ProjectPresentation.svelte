        ...
        <tfoot>
          {#if discAmount > 0}
            ...
            <tr class="pz-disc" class:expired={discApplied === 0}>
              <td>{data.discount?.label?.[lang] || L.promptDiscount}{#if discShownDeadline} · {L.until} <strong>{fmtDate(discShownDeadline)}</strong>{/if}{#if discApplied === 0} ({L.expired}){/if}</td>
