<script>
  ...
  // El idioma sale del componente (bind:lang) para que la página que lo envuelve —con
  // sus estados de carga y el acceso interno— hable el mismo que el proyecto.
  export let lang = 'es';
  let lightbox = null;
  let drafts = {};
  let saving = false;
  ...
  $: if (admin && data && (!ebInit || lang !== ebLang)) { ebInit = true; ebLang = lang; eb = {
    ...
    synopsis: (data.synopsis && data.synopsis[lang]) || '',
    ...
    pausedOn: !!data.paused, pausedReason: data.paused_reason || '',
    validUntil: data.proposal_valid_until || '',
    payTerms: data.payment_terms?.[lang] || '',
    ivaOn: Number(data.iva_rate ?? 0.21) > 0,
    ivaPct: String(Math.round(Number(data.iva_rate ?? 0.21) * 10000) / 100 || 21),
  ...
  $: docs = data?.docs || [];
  // Estado del trámite por tipo de documento (lo devuelve get_client_project como `states`).
  $: states = data?.states || {};
  ...
  // IVA/IRPF activables por proyecto (clientes extranjeros: sin impuestos) y con
  // porcentaje editable (facturación desde sucursal en otro país). Tipo 0 = impuesto
  // desactivado: su fila desaparece del presupuesto.
  $: ivaRate = admin && eb.title !== undefined
    ? (eb.ivaOn ? amt(eb.ivaPct) / 100 : 0)
    : Number(data?.iva_rate ?? 0.21);
  $: irpfRate = admin && eb.title !== undefined
    ? (eb.irpfOn ? amt(eb.irpfPct) / 100 : 0)
    : Number(data?.irpf_rate ?? 0.15);
  $: pctStr = (r) => (Math.round(r * 10000) / 100).toLocaleString(lang === 'es' ? 'es-ES' : 'en-GB');
  $: ivaLabel = (lang === 'es' ? 'IVA' : 'VAT') + ` (${pctStr(ivaRate)}%)`;
  $: irpfLabel = `IRPF (−${pctStr(irpfRate)}%)`;
  ...
  // Vigencia: la oferta se aplica entera hasta la fecha límite; después pierde 1.000 €
  // por semana transcurrida (empezada) hasta extinguirse. Es la misma regla que promete
  // el aviso de vencimiento por email (admin/cron_oferta.php): mantener ambos en sincronía.
  $: discAt = (ms) => {
    if (discAmount <= 0) return 0;
    if (!discDeadline || ms <= discDeadline.getTime()) return discAmount;
    return Math.max(0, discAmount - 1000 * Math.ceil((ms - discDeadline.getTime()) / 604800000));
  };
  $: discValid = discAt(Date.now()) > 0;
  // Fecha de vencimiento que ve el cliente: cada reducción semanal es en la práctica
  // una oferta nueva, así que la fecha mostrada avanza una semana por cada tramo
  // (fin de la semana de decaimiento en curso). La fecha GUARDADA no cambia: el
  // decaimiento y el aviso por email siguen contando desde el vencimiento original.
  // Congelada al aprobar y sin avanzar cuando la oferta ya se extinguió.
  // Validez de la propuesta: pasada la fecha (fin del día), el botón de aprobación
  // se desactiva. En edición sigue lo tecleado; fuera, lo guardado.
  // Paralización: bloquea la aprobación y muestra al cliente el motivo en un aviso.
  $: isPaused = admin && eb.title !== undefined ? !!eb.pausedOn : !!data?.paused;
  $: pausedReasonText = admin && eb.title !== undefined ? eb.pausedReason : (data?.paused_reason || '');
  $: proposalValidUntil = (admin && eb.title !== undefined ? eb.validUntil : data?.proposal_valid_until) || '';
  $: proposalExpired = !!proposalValidUntil && new Date() > new Date(proposalValidUntil + 'T23:59:59');
  $: payTermsText = admin && eb.title !== undefined ? eb.payTerms : (data?.payment_terms?.[lang] || '');
  $: discShownDeadline = (() => {
    if (!discDeadline) return null;
    if (approved || discAmount <= 0) return discDeadline;
    const past = Date.now() - discDeadline.getTime();
    if (past <= 0) return discDeadline;
    const weeks = Math.ceil(past / 604800000);
    return discAt(Date.now()) > 0 ? new Date(discDeadline.getTime() + weeks * 604800000) : discDeadline;
  })();
  // Al aprobar se congela: si se aprobó con oferta, vale lo que la oferta valía el día de
  // la aprobación; si no, desaparece.
  $: discApplied = approved
    ? (data?.approved_with_offer ? discAt(data?.approved_at ? new Date(data.approved_at).getTime() : Date.now()) : 0)
    : discAt(Date.now());
  ...
  /* Descarga del archivo ORIGINAL. Lo que se ve de Google Drive es una reducción
   * (thumbnail?id=…&sz=w2000); el archivo entero se pide por uc?export=download. Lo
   * que está en nuestro almacén se sirve tal cual, pidiéndole que llegue como
   * adjunto (?download) en vez de abrirse en otra pestaña. */
  const downloadUrl = (src) => {
    const id = driveId(src);
    if (id) return `https://drive.google.com/uc?export=download&id=${id}`;
    if (typeof src !== 'string' || !src) return src;
    return src + (src.includes('?') ? '&' : '?') + 'download';
  };
  ...
  async function doSaveBilling() {
    ...
    catch (e) { billingMsg = L.errGeneric; } finally { savingBilling = false; }
  }
  ...
  // Todo guardado admin pasa por aquí: si la sesión PHP caducó, deja de fallar en
  // silencio — se avisa en la barra y se notifica a la página para reabrir el login.
  function saveResult(r) {
    // El servidor marca offer_notified / valid_until_notified cuando el guardado cambió
    // una fecha que compromete al cliente y por eso salió un correo: quien edita tiene
    // que verlo, y saber CUÁL de las dos se ha comunicado.
    if (r && r.ok) {
      adminMsg = (r.offer_notified && r.valid_until_notified) ? L.datesNotified
        : r.offer_notified ? L.offerNotified
        : r.valid_until_notified ? L.validNotified
        : L.saved;
      return true;
    }
    adminMsg = r && r.error === 'unauthorized' ? L.sessionExpired : L.errGeneric;
    if (r && r.error === 'unauthorized') dispatch('expired');
    return false;
  }
  async function saveFields() {
    saving = true; adminMsg = '';
    const f = {}; f['title_' + lang] = eb.title; f['memoria_' + lang] = eb.memoria; f['synopsis_' + lang] = eb.synopsis || '';
    ...
    f.paused = eb.pausedOn ? '1' : '0'; f.paused_reason = eb.pausedReason || '';
    f.proposal_valid_until = eb.validUntil || '';
    f['payment_terms_' + lang] = eb.payTerms || '';
    f.iva_rate = eb.ivaOn ? String(amt(eb.ivaPct) / 100) : '0';
    f.irpf_rate = eb.irpfOn ? String(amt(eb.irpfPct) / 100) : '0';
  ...
  // Serializado: los tres campos autoguardan al perder el foco y dos peticiones en
  // vuelo podían llegar desordenadas al servidor (la lenta, con datos viejos, pisaba
  // a la rápida y la oferta "volvía" a su primer valor). Con el candado, si llega un
  // guardado mientras otro está en vuelo, se anota y se relanza al terminar con el
  // buffer ya al día.
  let discSaving = false, discQueued = false;
  async function saveDiscount() {
    if (discSaving) { discQueued = true; return; }
    discSaving = true;
    try {
  ...
  // El servidor marca price_notified cuando el cambio ha bajado el subtotal y por eso
  // ha salido el correo de «mejor precio» al cliente: quien edita tiene que saberlo.
  function budgetOk(r) {
    if (!adminOk(r)) return false;
    adminMsg = r.price_notified ? L.priceNotified : '';
    return true;
  }
  async function editBudget(item) {
    const f = { item_id: item.id, amount: item.amount };
    f['concept_' + lang] = item.concept[lang];
    budgetOk(await adminAction(token, 'edit_budget', f));
    await reload();
  }
  ...
  async function addBudget() {
    ...
    if (budgetOk(await adminAction(token, 'add_budget', f))) nb = { concept: '', amount: '' };
    await reload();
  }
  async function delBudget(id) { budgetOk(await adminAction(token, 'del_budget', { item_id: id })); await reload(); }
  ...
  $: uploadErrMsg = {
    unauthorized: L.upErrUnauthorized,
    file_too_big: L.upErrServerLimit,
    too_big: L.upErrTooBig,
    too_big_post: L.upErrTooBigPost,
    bad_mime: L.upErrBadMime,
    partial: L.upErrPartial,
    storage: L.upErrStorage
  };
  async function uploadFiles(fileList) {
    ...
    for (const file of files) {
      ...
      if (!r || !r.ok) {
        ...
        adminMsg = (uploadErrMsg[code] || (L.errUploading + ' ' + file.name)) + (code === 'unauthorized' ? '' : ' [' + file.name + ']');
  ...
  async function notifyClient() {
    ...
    saving = false; adminMsg = r && r.ok ? L.sentInternal : L.errNotify;
  }
</script>
...
<main class="pz" class:pz-admin={admin}>
  ...
  {#if isPaused}
    <div class="pz-paused-banner" role="status">
      <strong>⏸ {L.pausedTitle}</strong>{#if pausedReasonText} · {L.pausedReasonLabel}: {pausedReasonText}{/if}
      <span class="pz-paused-note">{L.pausedClientNote}</span>
    </div>
  {/if}
  <header class="pz-top">
    <div class="pz-id">
    ...
    <div class="pz-toggle" role="group" aria-label={L.langAria}>
  ...
  {#if admin}
    <div class="pz-admin-field">
      ...
      <input id="pz-client-email" class="pz-edit" type="email" multiple bind:value={eb.clientEmail} placeholder={L.clientEmailPh} />
    </div>
  {/if}

  <!-- Sinopsis (18/09/2026): texto enriquecido (estilos y listas) entre el interlocutor y
       la propuesta gráfica. Editable en ES y EN desde el modo edición; el servidor limpia
       el HTML con una lista blanca (cpx_clean_html). -->
  {#if admin}
    <section class="pz-block pz-synopsis-block">
      <h2 class="pz-h2">{L.synopsis}</h2>
      <RichEditor bind:value={eb.synopsis} placeholder={L.synopsisPh} {lang} />
    </section>
  {:else if String((data.synopsis && data.synopsis[lang]) || '').trim()}
    <section class="pz-block pz-synopsis-block">
      <h2 class="pz-h2">{L.synopsis}</h2>
      <div class="pz-synopsis" class:pz-blur={demo}>{@html data.synopsis[lang]}</div>
    </section>
  {/if}

  <section class="pz-block pz-media-block">
    {#if !BRAND.leadGen}
      <a class="pz-guarantee-stamp" href="https://standarte.es/proyecto-auditado" aria-label={L.guaranteeAria} target="_blank" rel="noopener">
      ...
      <article class="pz-media">
        ...
        >
          ...
          {:else if m.src}
            ...
            {#if !admin && isLbItem(m)}
              <span class="pz-hints">
                <!-- La descarga es un enlace de verdad, así que hay que impedir que el
                     clic (o el Enter) llegue a la tarjeta y abra además la ampliación. -->
                {#if m.type === 'image'}
                  <a class="pz-dl-hint" href={downloadUrl(m.src)} target="_blank" rel="noopener" download
                     title={L.dlImage} aria-label={L.dlImage}
                     on:click|stopPropagation={() => {}} on:keydown|stopPropagation={() => {}}>⤓</a>
                {/if}
          ...
          {#if admin}
            ...
            <div class="pz-order">
              <button class="pz-ord-btn" title={L.moveUp} disabled={mi === 0 || reordering} on:click|stopPropagation={() => moveMedia(m, -1)}>↑</button>
              <span class="pz-ord-n">{mi + 1}</span>
              <button class="pz-ord-btn" title={L.moveDown} disabled={mi === data.media.length - 1 || reordering} on:click|stopPropagation={() => moveMedia(m, 1)}>↓</button>
            </div>
          {/if}
        </div>
  ...
  <section class="pz-block">
        ...
        <tfoot>
          {#if discAmount > 0}
            ...
            <tr class="pz-disc" class:expired={discApplied === 0}>
              <td>{data.discount?.label?.[lang] || L.promptDiscount}{#if discShownDeadline} · {L.until} <strong>{fmtDate(discShownDeadline)}</strong> {L.inclusive}{/if}{#if discApplied === 0} ({L.expired}){/if}</td>
          ...
          {#if ivaRate > 0}<tr><td>{ivaLabel}</td><td class="num">+ {fmt(iva)}{#if usdRate}<span class="pz-usd">+ {fmtUsd(iva)}</span>{/if}</td>{#if admin}<td></td>{/if}</tr>{/if}
          {#if irpfRate > 0}<tr><td>{irpfLabel}</td><td class="num">− {fmt(irpf)}{#if usdRate}<span class="pz-usd">− {fmtUsd(irpf)}</span>{/if}</td>{#if admin}<td></td>{/if}</tr>{/if}
          <tr class="grand"><td>{L.total}</td><td class="num"><span class:pz-blur={demo}>{fmt(total)}</span></td>{#if admin}<td></td>{/if}</tr>
          {#if usdRate}
            <tr class="pz-usd-row"><td>{L.totalUsd}</td><td class="num"><span class:pz-blur={demo}>{fmtUsd(total)}</span></td>{#if admin}<td></td>{/if}</tr>
          {/if}
        </tfoot>
      </table>
      {#if proposalValidUntil || payTermsText}
        <dl class="pz-budget-meta">
          {#if proposalValidUntil}
            <div class:pz-meta-expired={proposalExpired}>
              <dt>{L.validUntilLabel}</dt>
              <dd>{fmtDate(new Date(proposalValidUntil + 'T12:00:00'))}{#if proposalExpired} · <strong>{L.proposalExpired}</strong>{/if}</dd>
            </div>
          {/if}
          {#if payTermsText}
            <div><dt>{L.payTerms}</dt><dd>{payTermsText}</dd></div>
          {/if}
        </dl>
      {/if}
    ...
    {#if admin}
      ...
      <label class="pz-elabel" style="margin-top:12px;display:block">{L.validUntilLabel}</label>
      ...
      <div class="pz-tax-row">
        ...
        {#if eb.ivaOn}<input class="pz-edit pz-tax-pct" inputmode="decimal" bind:value={eb.ivaPct} /><span class="pz-tax-sign">%</span>{/if}
      </div>
      <div class="pz-tax-row">
        <label class="pz-paidtoggle pz-taxtoggle"><input type="checkbox" bind:checked={eb.irpfOn} /> {L.applyIrpf}</label>
        {#if eb.irpfOn}<input class="pz-edit pz-tax-pct" inputmode="decimal" bind:value={eb.irpfPct} /><span class="pz-tax-sign">%</span>{/if}
      </div>
      <p class="pz-drive-hint">{L.taxHint}</p>
      <label class="pz-paidtoggle"><input type="checkbox" bind:checked={eb.paid} /> {L.paid}</label>
      <div class="pz-pause-edit">
        <label class="pz-paidtoggle pz-pausetoggle"><input type="checkbox" bind:checked={eb.pausedOn} /> {L.pausedLabel}</label>
        {#if eb.pausedOn}<input class="pz-edit" bind:value={eb.pausedReason} placeholder={L.pausedReasonPh} />{/if}
      </div>
      {#if approved}
    ...
    {:else}
      <div class="pz-approve-wrap">
        ...
        <button class="pz-approve" class:pz-approve-offer={discValid && !proposalExpired && !isPaused} type="button" on:click={approve} disabled={approving || demo || proposalExpired || isPaused}>
          {approving ? '…' : (discValid && !proposalExpired ? L.approveWithOffer : L.approve)}
        </button>
        {#if proposalExpired}<p class="pz-proposal-expired">{L.proposalExpiredNote}</p>{/if}
      </div>
    {/if}
  </section>
          ...
          {#if lightbox.src}
            ...
            {:else}
              ...
              <div class="pz-lb-video">
                ...
                {#if !lbPlaying}
                  <button type="button" class="pz-lb-video-play" on:click={playLbVideo} aria-label={L.playVideo}><span aria-hidden="true">▶</span></button>
                {/if}
              </div>
            {/if}
          {:else}
...
<style>
  ...
  .pz-synopsis { font-size: 16px; line-height: 1.6; color: #2a2a28; }
  .pz-synopsis :global(p) { margin: 0 0 10px; }
  .pz-synopsis :global(h3) { font-size: 18px; margin: 16px 0 6px; }
  .pz-synopsis :global(ul), .pz-synopsis :global(ol) { margin: 0 0 10px; padding-left: 24px; }
  .pz-synopsis :global(li) { margin: 0 0 4px; }
  ...
  .pz-hints { position: absolute; top: 8px; right: 8px; display: flex; align-items: center; gap: 10px; z-index: 2; }
  .pz-zoom-hint { font-size: 18px; color: #fff; text-shadow: 0 1px 4px rgba(0,0,0,0.5); opacity: 0.85; }
  .pz-dl-hint { font-size: 18px; line-height: 1; color: #fff; text-shadow: 0 1px 4px rgba(0,0,0,0.5); opacity: 0.85; text-decoration: none; cursor: pointer; }
  .pz-dl-hint:hover, .pz-dl-hint:focus-visible { opacity: 1; color: #ffc800; }
  ...
  .pz-paused-banner {
    ...
    background: #fff4e5; border: 1px solid #f0c36d; color: #7a4d00;
  }
  .pz-paused-banner .pz-paused-note { display: block; margin-top: 4px; font-size: 0.92em; }
  .pz-pause-edit { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
  .pz-budget-meta { margin: 10px 0 0; display: flex; flex-direction: column; gap: 4px; }
  .pz-budget-meta div { display: flex; gap: 8px; align-items: baseline; }
  .pz-budget-meta dt { font-weight: 700; }
  .pz-budget-meta dt::after { content: ':'; }
  .pz-budget-meta dd { margin: 0; }
  .pz-meta-expired dd { color: #c0392b; }
  .pz-proposal-expired { margin: 8px 0 0; color: #c0392b; font-weight: 600; }
  .pz-usd-row td { color: #6b6f75; font-weight: 600; font-size: 0.92em; }
  .pz-tax-row { display: flex; align-items: center; gap: 8px; }
  .pz-taxtoggle { margin: 4px 0; }
  .pz-tax-pct { width: 72px; text-align: right; }
  .pz-tax-sign { color: #6b6f75; font-weight: 600; }
